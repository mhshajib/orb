...
interface Body { backup_code?: string }
...
export default defineEventHandler(async (event) => {
  ...
  const res = await callBackend<{ data: TokenSuccess }>(event, '/api/auth/2fa/backup', {
    method: 'POST',
    body: { session_token, backup_code: body.backup_code },
    auth: false,
  })
